...
def _render_amo_settings(store, runtime: dict[str, Any]) -> None:
    ...
    with st.expander("Где взять настройки amoCRM", expanded=False):
        st.markdown(
            "1. **Домен:** скопируйте адрес аккаунта без `https://` и путей, "
            "например `company.amocrm.ru`.\n"
            "2. **Access token:** откройте **amoМаркет → Установленные → Создать интеграцию → "
            "Внешняя интеграция**. После создания откройте интеграцию и скопируйте "
            "долгосрочный Access Token. Нужны права на сделки, контакты, компании, "
            "задачи и файлы.\n"
            "3. **ID воронки и этапа:** временно укажите любые числа, сохраните настройки "
            "и нажмите **Проверить подключение**. При неверных значениях проверка покажет "
...
def _render_bitrix_settings(store, runtime: dict[str, Any]) -> None:
    ...
    with st.expander("Где взять webhook и ID", expanded=False):
        st.markdown(
            "1. **Webhook:** откройте **Приложения → Ресурсы разработчика → Другое → "
            "Входящий webhook**. Разрешите доступ к **CRM** и **Задачам**, сохраните и "
            "скопируйте полный URL вида `https://portal.bitrix24.kz/rest/1/secret/`.\n"
            "2. **ID воронки:** для основной воронки используйте `0`. Для дополнительной "
            "воронки нажмите **Проверить подключение** с неверным ID — система покажет "
            "доступные воронки и их ID.\n"
            "3. **ID этапа:** для первого этапа основной воронки обычно используется "
            "`NEW`; для дополнительной — значение вида `C7:NEW`. Проверка покажет "
            "доступные этапы.\n"
        ...
        st.warning(
            ...
            "отправлен постороннему, пересоздайте webhook."
        )
