...
amoCRM на этапе X3-dev.2 поддерживает сохранение параметров и проверку подключения. Создание контактов, компаний, сделок, файлов и задач относится к X3-dev.3. Bitrix24 подключается следующим адаптером после приемки amoCRM.
...
1. `python -m py_compile audit_app.py crm_admin.py crm_store.py crm_assets.py`
2. Запустить smoke-тесты черновика, анкеты, генерации и CRM-админки.
3. Проверить загрузку и повторное применение JSON.
4. Проверить презентацию в PowerPoint без восстановления файла.
5. Проверить Excel, Telegram, бренд и портфель.
6. Проверить мобильный экран и светлую тему.
7. Сверить номер версии и CHANGELOG_CUSTOMER.md.
8. Выполнить ручную приемку Test.
