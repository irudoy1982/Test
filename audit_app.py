...
st.title("📋 Опросник: Технический аудит ИТ и ИБ (2026) v6.0 (Logic Update)")
...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    **Новое в v6.0:** Внедрена система автоматической проверки логики данных. Если суммы в разделах не совпадают, система подсветит ошибку.
    """)
...
# Логика 1: Проверка АРМ
if total_arm > 0 and sum_os_arm != total_arm:
    st.warning(f"⚠️ Ошибка в расчетах АРМ: Указано всего {total_arm}, а по ОС набралось {sum_os_arm}. Разница: {abs(total_arm-sum_os_arm)}")
...
total_srv_calc = phys_count + virt_count
...
# Логика 2: Проверка Серверов
if total_srv_calc > 0 and sum_os_srv != total_srv_calc:
    st.warning(f"⚠️ Ошибка в блоке Серверов: Физические + Виртуальные = {total_srv_calc}, а по списку ОС распределено {sum_os_srv}.")
    validation_errors.append("Несовпадение количества Серверов и ОС")
...
if st.checkbox("Резервное копирование", key="ib_backup"):
    v_n_b = st.text_input("Вендор Резервного копирования (Veeam и др.):", key="vn_backup")
...
if st.toggle("Есть собственная СХД", key="storage_toggle"):
    st_media_sel = st.multiselect("Типы носителей:", ["HDD (NL-SAS / SATA)", "SSD (SATA / SAS)", "NVMe", "SCM (Storage Class Memory)"], key="st_media")
    ...
    # Логика 3: Если СХД есть, а дисков 0
    if st_media_sel and (cnt_hdd + cnt_ssd == 0):
        st.info("ℹ️ Вы указали наличие СХД, но не ввели количество дисков.")
...
if st.toggle("Разработка", key="dev_toggle"):
    ...
    with col_d2:
        ...
        # Логика 4: Проверка Разработки
        if dev_count > 0 and not sel_langs:
            st.info("ℹ️ Указаны разработчики, но не выбран стек технологий.")
        if dev_count == 0 and sel_langs:
            st.warning("⚠️ Выбраны языки программирования, но количество разработчиков — 0.")
            validation_errors.append("Противоречие в блоке разработки")
...
if validation_errors:
    st.error(f"🚨 Формирование отчета недоступно. Обнаружено ошибок в логике: {len(validation_errors)}")
    st.info("Пожалуйста, проверьте предупреждения (желтые блоки) в разделах АРМ, Серверы или Разработка.")
...
st.info("Khalil Audit System v6.0 | Almaty 2026")
