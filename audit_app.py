...
if selected_os_arm:
    for os_item in selected_os_arm:
        data[f"ОС АРМ ({os_item})"] = st.number_input(f"Кол-во АРМ на {os_item}:", min_value=0, step=1)

# 1.2 Сетевая инфраструктура
st.write("---")
st.subheader("1.2. Сетевая инфраструктура")
if st.toggle("Своя сетевая инфраструктура", key="net_tgl"):
    net_types = ["Оптика", "Радиорелейная", "Спутник", "4G/5G", "ADSL", "Нет"]
    c_n1, c_n2 = st.columns(2)
    with c_n1:
        m_t = st.selectbox("Основной канал:", net_types)
        m_s = st.number_input("Скорость осн. (Mbit/s):", min_value=0, step=10)
        data['1.2.1. Основной канал'] = f"{m_t} ({m_s} Mbps)"
    with c_n2:
        b_t = st.selectbox("Резервный канал:", net_types)
        b_s = st.number_input("Скорость рез. (Mbit/s):", min_value=0, step=10)
