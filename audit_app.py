...
st.title("📋 Опросник: Технический аудит ИТ и ИБ (2026) v5.1 (Стабильная)")
...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    **1. Подготовка к заполнению**
    Подготовьте данные о количестве АРМ, параметрах интернет-каналов, серверном парке (физическом и виртуальном) и дисковых массивах.

    **2. Заполнение разделов**
    * **Общая информация:** Укажите сайт и телефон. Система настроит Email на базе домена.
    * **ИТ-инфраструктура:** Подробно заполните данные по сети, СХД (в шт. дисков) и серверным ОС.
    * **Безопасность:** Отметьте используемые инструменты защиты.
    * **Разработка:** Укажите стек технологий и количество специалистов.
...
with sc1:
    data['1.3.1. Физические серверы'] = st.number_input("Кол-во физических серверов (шт):", min_value=0, step=1)
with sc2:
    data['1.3.2. Виртуальные машины'] = st.number_input("Кол-во виртуальных серверов (ВМ):", min_value=0, step=1)
...
if st.checkbox("Резервное копирование"):
    data["Бэкап система"] = st.text_input("Вендор Бэкапа:")
    score += 20
...
if st.toggle("Есть собственная СХД", key="storage_toggle"):
    st.write("**Количество дисков по типам:**")
    st_c1, st_c2, st_c3, st_c4 = st.columns(4)
    with st_c1:
        data['1.4.1. Диски HDD (шт)'] = st.number_input("HDD:", min_value=0, step=1)
    with st_c2:
        data['1.4.2. Диски SSD (шт)'] = st.number_input("SSD:", min_value=0, step=1)
    with st_c3:
        data['1.4.3. Диски NVMe (шт)'] = st.number_input("NVMe:", min_value=0, step=1)
    with st_c4:
        data['1.4.4. Диски SCM (шт)'] = st.number_input("SCM:", min_value=0, step=1)
    
    col_st_chk1, col_st_chk2 = st.columns(2)
    data['1.4.5. Гибридная СХД'] = col_st_chk1.checkbox("Гибридная архитектура")
    data['1.4.6. All-Flash'] = col_st_chk2.checkbox("All-Flash массив")
    data['1.4.7. RAID'] = st.multiselect("RAID-группы:", ["RAID 0", "RAID 1", "RAID 5", "RAID 6", "RAID 10", "RAID 50", "RAID 60"])
    ...
    for i, (label, pts) in enumerate(items):
        ...
        with target_col:
            if st.checkbox(label, key=f"ib_{label}"):
                ...
                data[label] = "Нет"
    data['Примечание (ИБ)'] = st.text_area("Примечания по ИБ:", placeholder="Особенности ИБ-защиты...")
...
if st.toggle("Web-ресурсы", key="web_toggle"):
    ...
    data['Примечание (Web)'] = st.text_area("Примечания по Web:", placeholder="Стек, CMS...")
...
def make_expert_excel(c_info, results, final_score):
    ...
    for k, v in results.items():
        ...
        status, rec = "В норме", "Поддерживать."
        is_risk = False
        if "Примечание" in k:
            status, rec = "Инфо", "Анализ."
        elif "Нет" in str(v) or v == 0:
            is_risk = True; status = "РИСК"; rec = "Внедрить."
...
st.info("Khalil Audit System v5.1 | Almaty 2026")
