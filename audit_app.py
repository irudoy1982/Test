...
with col_h1
    ...
    if custom_email_mode
        ...
        if clean_domain and . in clean_domain
            ...
            with e_col2
                st.markdown(fdiv style='padding-top 5px; font-weight bold; color #1F4E78;'@{clean_domain}div, unsafe_allow_html=True)
...
selected_os_arm = st.multiselect(ОС на АРМ, [Windows XPVista78, Windows 10, Windows 11, Linux, macOS, Другое])

sum_os_arm = 0
if selected_os_arm
    for os_item in selected_os_arm
        val = st.number_input(fКол-во на {os_item}, min_value=0, step=1, key=farm_{os_item})
        data[fОС АРМ ({os_item})] = val
        sum_os_arm += val
...
if st.toggle(Своя сетевая инфраструктура, key=net_toggle)
    net_types = [Оптика, Радиорелейная, Спутник, 4G5G, Starlink, ADSLVDSL, Нет]
    ...
    with c_net1
        if st.checkbox(Маршрутизаторы, key=router_chk)
            ...
            data['1.2.4. Маршрутизаторы'] = fДа ({r_count} шт)
    with c_net2
        if st.checkbox(Коммутаторы L2, key=swl2_chk)
            ...
            data['1.2.5. Коммутаторы L2'] = fДа ({sw2_count} шт)
    with c_net3
        if st.checkbox(Коммутаторы L3, key=swl3_chk)
            ...
            data['1.2.6. Коммутаторы L3'] = fДа ({sw3_count} шт)
    ...
    if st.checkbox(Wi-Fi, key=wifi_toggle)
        ...
        with w_col1
            if st.checkbox(Контроллер, key=wifi_ctrl)
                wc_v = st.text_input(Производительмодель контроллера, key=wc_vendor)
        ...
        with w_col3
            wf_types = [Wi-Fi 66E (802.11ax), Wi-Fi 5 (802.11ac), Wi-Fi 4 (802.11n), Другое]
    ...
    if st.checkbox(Межсетевой экран (NGFW), key=ngfw_chk)
        ...
        data['1.2.7. NGFW'] = fДа ({ngfw_vendor if ngfw_vendor else 'не указан'})
        score += 20
...
s_os_list = [Windows Server 20082012 R2, Windows Server 2016, Windows Server 2019, Windows Server 2022, Linux, Unix, Другое]
selected_os_srv = st.multiselect(Выберите ОС серверов, s_os_list, key=ms_srv_list)
sum_os_srv = 0
if selected_os_srv
    for os_s in selected_os_srv
        val_os = st.number_input(fКол-во на {os_s}, min_value=0, key=fsrv_{os_s})
        data[fОС Сервера ({os_s})] = val_os
        sum_os_srv += val_os
...
if selected_virt_sys and Нет not in selected_virt_sys
    for v_sys in selected_virt_sys
        data[fСистема виртуализации ({v_sys})] = st.number_input(fКоличество хостов {v_sys}, min_value=0, step=1, key=fv_cnt_{v_sys})

if st.checkbox(Резервное копирование, key=ib_backup)
    v_n_b = st.text_input(Вендор Резервного копирования, key=vn_backup)
    data[Резервное копирование] = fДа ({v_n_b if v_n_b else 'не указан'})
    score += 20
...
if st.toggle(ИС организации, key=is_toggle)
    ...
    # ПРАВКА Выбор версии для On-Prem решений
    if m_sys in [Exchange (On-Prem), Lotus]
        m_ver = st.text_input(fВерсия {m_sys}, key=mail_version_input)
    ...
    for label, ks in is_list.items()
        if st.checkbox(label)
            data[fИС {label}] = st.text_input(fВерсия {label}, key=fver_{ks})
...
if st.toggle(Средства защиты, key=ib_toggle)
    # ДОБАВЛЕНО MFA и Anti-DDoS в список систем ИБ
    ib_systems = {
        EPP (Антивирус) 10, DLP (Утечки) 15, PAM (Привилегии) 10,
        SIEM (Мониторинг) 20, VM (Уязвимости) 10, EDRXDR (Точки) 15,
        WAF (Веб) 10, Sandbox (Песочница) 5, IDSIPS (Атаки) 5, IDMIGA (Доступ) 5,
    ...
    for i, (label, pts) in enumerate(items)
        ...
        with target_col
            if st.checkbox(label, key=fib_{label})
                v_n = st.text_input(fВендор {label}, key=fvn_{label})
                data[label] = fДа ({v_n if v_n else 'не указан'})
                score += pts
            else
                data[label] = Нет
...
if st.toggle(Разработка, key=dev_toggle)
    ...
    with col_d1
        ...
        data['4.2. CICD'] = st.checkbox(Используется CICD)
    with col_d2
        lang_list = [Python, JavaScriptTypeScript, Java, C#  .NET, PHP, Go, C++, SwiftKotlin, Другое]
...
def make_expert_excel(c_info, results, final_score)
    ...
    for k, v in results.items()
        ...
        if (20082012 R2 in str(k) or XP in str(k)) and v  0
...
if st.button(📊 Сформировать экспертный отчет, disabled=len(validation_errors)  0)
    ...
    if not all(mandatory)
        ...
        with st.spinner(Обработка...)
            ...
            try
                caption = f🚀 Новый аудит Khalil Traden🏢 {client_info['Наименование компании']}n📊 Зрелость {f_score}%
                requests.post(fhttpsapi.telegram.orgbot{TOKEN}sendDocument, 
                              data={chat_id CHAT_ID, caption caption, parse_mode Markdown}, 
                              files={'document' (fAudit_{client_info['Наименование компании']}.xlsx, report_bytes)})
            except pass
            st.success(Отчет готов!)
            st.download_button(📥 Скачать отчет, report_bytes, fAudit_{client_info['Наименование компании']}.xlsx)
