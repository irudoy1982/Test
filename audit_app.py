...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    4.  **Результат:** После заполнения нажмите кнопку «Сформировать экспертный отчет».
    """)
...
with col_it1:
    ...
    for os_item in selected_os:
        val = st.number_input(f"Кол-во {os_item}:", min_value=0, step=1, key=f"arm_{os_item}")
        sum_os += val
...
with col_it2:
    ...
    if it_lic == "Да": 
        ...
        add_audit_item("ИТ", "Лицензионное ПО", "РИСК", "Провести инвентаризацию и лицензирование ПО", True)
    else: 
        score += 5
        add_audit_item("ИТ", "Лицензионное ПО", "ВНИМАНИЕ", "Завершить лицензирование")
...
with col_ib1:
    ...
    if ib_mfa:
        ...
        add_audit_item("ИБ", "MFA", "КРИТИЧЕСКИЙ РИСК", "Необходимо внедрить MFA для защиты от компрометации учетных записей", True)
    ...
    if ib_ddos:
        ...
        add_audit_item("ИБ", "Anti-DDoS", "РИСК", "Рекомендуется подключение Anti-DDoS для обеспечения доступности сервисов", True)
...
with col_ib2:
    ...
    if ib_av:
        ...
        add_audit_item("ИБ", "Антивирусная защита", "КРИТИЧЕСКИЙ РИСК", "Требуется установка централизованного антивируса/EDR", True)
...
def make_expert_excel(c_info, audit_data, final_score):
    ...
    cols = ["Категория", "Объект аудита", "Статус", "Рекомендация"]
    ws.append(cols)
    for cell in ws[5]:
