...
st.title("📋 Опросник: Технический аудит ИТ и ИБ (2026) v4.6")
...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    **1. Подготовка к заполнению**
    Подготовьте данные о количестве АРМ, параметрах интернет-каналов, серверном парке и версиях ключевых систем (1С, ERP, CRM).

    **2. Заполнение разделов**
    * **Общая информация:** Укажите сайт и телефон. Система настроит Email на базе домена.
    * **ИТ-инфраструктура:** Отразите параметры связи, типы СХД и версии серверных ОС.
    * **Безопасность:** Отметьте используемые инструменты (DLP, SIEM, VM, EDR) для расчета индекса зрелости.
    * **Примечания:** В конце каждого активного блока вы можете указать дополнительные сведения.
...
if st.toggle("Своя сеть", key="net_toggle"):
    ...
    st.write("**Доп. каналы и Оборудование:**")
    ...
    if l1.checkbox("Ядро (Core)"): data['Уровень сети: Ядро'] = st.text_input("Вендор Core:")
    if l2.checkbox("Распределение (Dist)"): data['Уровень сети: Распределение'] = st.text_input("Вендор Dist:")
    if l3.checkbox("Доступ (Access)"): data['Уровень сети: Доступ'] = st.text_input("Вендор Access:")

    if st.checkbox("Wi-Fi"):
        data['Wi-Fi Точки'] = st.number_input("Кол-во точек:", min_value=0)
        data['Wi-Fi Контроллер'] = st.text_input("Модель контроллера:")

    if st.checkbox("Межсетевой экран (NGFW)"):
        data['1.2.7. NGFW'] = st.text_input("Вендор NGFW:")
        score += 20
...
if st.checkbox("Резервное копирование"):
    data["Бэкап"] = st.text_input("Вендор Бэкапа:")
    score += 20
...
if st.toggle("Есть СХД", key="st_t"):
    data['1.4.1. Носители'] = st.multiselect("Типы:", ["HDD", "SSD", "NVMe", "SCM"])
    data['1.4.6. RAID'] = st.multiselect("RAID:", ["RAID 0", "RAID 1", "RAID 5", "RAID 6", "RAID 10", "RAID 50", "RAID 60"])
...
if st.toggle("ИС организации", key="is_t"):
    ...
    # Свободный ввод для Блока 1 (появляется только если ИС или Своя сеть включены)
    data['Примечание (ИТ)'] = st.text_area("Примечания по ИТ-инфраструктуре:", placeholder="Укажите то, что мы не учли...")
...
if st.toggle("Средства защиты", key="ib_toggle"):
    ...
    # Свободный ввод для Блока 2
    data['Примечание (ИБ)'] = st.text_area("Примечания по Информационной Безопасности:", placeholder="Опишите специфику ваших средств защиты...")
...
if st.toggle("Web-ресурсы", key="web_toggle"):
    ...
    # Свободный ввод для Блока 3
    data['Примечание (Web)'] = st.text_area("Примечания по Web-ресурсам:", placeholder="Особенности сайтов, CMS или внешних сервисов...")
...
if st.toggle("Разработка", key="dev_toggle"):
    ...
    # Свободный ввод для Блока 4
    data['Примечание (Разработка)'] = st.text_area("Примечания по Разработке:", placeholder="Методологии (Agile/Scrum), специфика стека и т.д.")
...
st.info("Khalil Audit System v4.6 | Almaty 2026")
