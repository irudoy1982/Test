...
total_arm = st.number_input("Общее количество АРМ (шт):", min_value=0, step=1, key="arm_total")
data['1.1. Всего АРМ'] = total_arm
data['1.4. Почта'] = st.selectbox("Тип почты:", ["Microsoft 365", "Google Workspace", "Exchange", "SmarterMail", "Нет"])

# БЛОК 2: СЕТЬ (из версии 1.0)
st.header("Блок 2: Сетевая инфраструктура")
if st.toggle("Показать настройки сети", key="net_tgl"):
    c_n1, c_n2 = st.columns(2)
    with c_n1:
        data['2.1. Основной канал'] = st.selectbox("Тип канала:", ["Оптика", "Радиорелейная", "Спутник", "4G/5G", "Starlink"])
        data['2.2. Скорость (Мбит/с)'] = st.number_input("Скорость канала:", min_value=0)
    with c_n2:
        data['2.3. Резервный канал'] = st.selectbox("Резерв:", ["Нет", "Оптика", "4G/5G", "Спутник"])
    
    data['2.4. NGFW'] = st.text_input("Вендор Межсетевого экрана (NGFW):")
    if data['2.4. NGFW']: score += 20
    
    if st.checkbox("Используется Wi-Fi?"):
        data['2.5. Wi-Fi Контроллер'] = st.text_input("Модель контроллера (если есть):")
        data['2.6. Кол-во точек'] = st.number_input("Количество точек доступа:", min_value=0)

# БЛОК 3: СЕРВЕРЫ
st.header("Блок 3: Серверная инфраструктура")
if st.toggle("Показать настройки серверов", key="srv_tgl"):
    col_s1, col_s2 = st.columns(2)
    data['3.1. Физические серверы'] = col_s1.number_input("Физических серверов:", min_value=0, step=1)
    data['3.2. Виртуальные серверы'] = col_s2.number_input("Виртуальных серверов:", min_value=0, step=1)
    data['3.3. Виртуализация'] = st.multiselect("Системы:", ["VMware", "Hyper-V", "Proxmox", "KVM", "Нет"])

# БЛОК 4: ИБ (из версии Б)
st.header("Блок 4: Информационная Безопасность")
ib_list = {"DLP": 15, "PAM": 10, "SIEM/SOC": 20, "WAF": 10, "EDR": 15, "Резервное копирование": 20}
...
# --- 5. ГЕНЕРАЦИЯ EXCEL (с Лого и Рекомендациями) ---
def make_expert_excel(c_info, results, final_score):
    ...
    ws.cell(row=curr_row, column=1, value="Дата аудита:").font = Font(bold=True)
    ...
    rec_map = {
        ...
        "NGFW": "Рекомендуется для защиты периметра сети.",
        "Email": "Обеспечить защиту почты (SPF, DKIM, DMARC)."
    }
    ...
    for k, v in results.items():
        ...
        rec_text = rec_map.get(k, "Рассмотреть внедрение.") if is_risk else "Поддерживать работоспособность."
        ws.cell(row=curr_row, column=4, value=rec_text).border = border
        curr_row += 1
    ...
    ws.column_dimensions['A'].width = 35; ws.column_dimensions['B'].width = 30; ws.column_dimensions['D'].width = 50
    wb.save(output)
...
if st.button("📊 Сформировать экспертный отчет", use_container_width=True):
    ...
    if not all(mand):
        ...
        with st.spinner("Генерация..."):
            ...
            try:
                msg = (f"🚀 *Коллеги, у нас новый заказ! Давайте зарабатывать!*\n\n"
