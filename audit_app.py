...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    * **ИТ-инфраструктура:** Отразите параметры связи (ЕШДИ/ЕТСГО), типы СХД и версии серверных ОС.
...
if selected_virt and "Нет" not in selected_virt:
    for v_s in selected_virt:
        data[f"Виртуализация ({v_s})"] = st.number_input(f"Хостов {v_s}:", min_value=0)
...
st.header("Блок 3: Web и Разработка")
...
def make_expert_excel(c_info, results, final_score):
    ...
    for k, v in results.items():
        ...
        if "2008/2012 R2" in str(k) and v > 0:
