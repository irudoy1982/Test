...
with col_h1:
    ...
    site_input = st.text_input("Сайт компании (необязательно):", key="site_field")
    client_info['Сайт компании'] = site_input

    # ЛОГИКА ФИКСАЦИИ ДОМЕНА EMAIL
    ...
    if clean_domain and "." in clean_domain:
        ...
        client_info['Email'] = st.text_input("Email контактного лица:*", placeholder="example@domain.kz")
...
if st.toggle("Своя сетевая инфраструктура", key="net_toggle"):
    ...
    data['2.1. Основной канал'] = st.selectbox("Тип канала:", net_types, key="net_type")
    data['2.4. NGFW'] = st.text_input("Вендор NGFW:", key="ngfw_v")
...
if st.toggle("Есть свои Web-ресурсы", key="web_toggle"):
    data['4.1. Хостинг'] = st.selectbox("Хостинг:", ["Собственный ЦОД", "Облако (KZ)", "Облако (Global)"], key="host")
    data['4.4. Frontend'] = st.multiselect("Frontend серверы:", ["Nginx", "Apache", "IIS", "LiteSpeed", "Caddy", "Cloudflare"], key="fnt")

# Блок 5: Разработка
st.header("Блок 5: Разработка")
if st.toggle("Своя разработка", key="dev_toggle"):
    data['5.1. Разработчики'] = st.number_input("Кол-во разработчиков:", min_value=0, key="dev_c")
    data['5.3. CI/CD'] = st.checkbox("CI/CD используется", key="cicd_c")


# --- ГЕНЕРАЦИЯ EXCEL (С АВТОМАТИЧЕСКОЙ ДАТОЙ) ---
def make_expert_excel(c_info, results, final_score):
    ...
    # Добавляем дату в Excel автоматически
    ...
    rec_map = {
        ...
        "CI/CD": "Автоматизация повысит качество кода."
    }
