...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    3.  **Логический контроль:** Система проверяет соответствие количества устройств и операционных систем.
...
with col_ib2:
    audit_question("ИБ", "Используется ли централизованный антивирус/EDR?", 15, 
                   "Централизованное управление ИБ позволяет оперативно реагировать на угрозы на всех узлах сети одновременно.")
    
    audit_question("ИБ", "Ограничены ли права локальных администраторов у пользователей?", 10, 
                   "Минимизация прав снижает риск случайного или намеренного заражения системы вредоносным ПО.")
