...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    4.  **Результат:** После заполнения нажмите кнопку «Сформировать экспертный отчет». Вы получите файл Excel с анализом рисков и обоснованием рекомендаций.
    """)
...
# Вспомогательная функция для формирования данных
...
with col_it2:
    ...
    if it_lic == "Да": 
        score += 10
        add_audit_item("ИТ", "Лицензионное ПО", "OK", "Соблюдается")
    elif it_lic == "Нет": 
        add_audit_item("ИТ", "Лицензионное ПО", "РИСК", "Необходимо провести лицензирование ПО для минимизации юридических рисков и получения обновлений безопасности.", True)
    else: 
        score += 5
        add_audit_item("ИТ", "Лицензионное ПО", "ВНИМАНИЕ", "Завершить процесс инвентаризации и лицензирования.")
...
with col_ib1:
    # MFA
    ib_mfa = st.checkbox("Многофакторная аутентификация (MFA)", key="ib_mfa")
    if ib_mfa:
        score += 20
        add_audit_item("ИБ", "MFA", "OK", "Внедрено")
    else:
        add_audit_item("ИБ", "MFA", "КРИТИЧЕСКИЙ РИСК", "Рекомендуется внедрение MFA. Обоснование: защита от компрометации учетных данных и предотвращение 99% атак, связанных с кражей паролей.", True)

    # Anti-DDoS
    ib_ddos = st.checkbox("Защита от DDoS-атак (Anti-DDoS)", key="ib_ddos")
    if ib_ddos:
        score += 15
        add_audit_item("ИБ", "Anti-DDoS", "OK", "Внедрено")
    else:
        add_audit_item("ИБ", "Anti-DDoS", "РИСК", "Рекомендуется подключение Anti-DDoS. Обоснование: обеспечение доступности внешних сервисов компании и защита от простоев при атаках на отказ в обслуживании.", True)

with col_ib2:
    # Антивирус/EDR
    ib_av = st.checkbox("Централизованный Антивирус / EDR", key="ib_av")
    if ib_av:
        score += 15
        add_audit_item("ИБ", "Антивирусная защита", "OK", "Внедрено")
    else:
        add_audit_item("ИБ", "Антивирусная защита", "КРИТИЧЕСКИЙ РИСК", "Требуется внедрение централизованного EDR-решения. Обоснование: защита конечных точек от современных шифровальщиков и продвинутых угроз (APT).", True)
...
def make_expert_excel(c_info, audit_data, final_score):
    ...
    ws.append([f"Уровень зрелости инфраструктуры: {final_score}%"])
    ws.append([])
