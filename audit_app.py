...
# --- DLP ---
if "нет" in str(results.get("DLP", "")).lower():
    vendor_recs.append(
        "Защита данных (DLP): Рекомендуется внедрение системы предотвращения утечек данных. "
        "Рассмотреть решения: Symantec, Forcepoint, Zecurion, ibatyr, Strac. "
        "Выбор зависит от требований к контролю каналов (почта, web, устройства) и глубины анализа данных."
    )

# --- ШИФРОВАНИЕ ---
if "нет" in str(results.get("Шифрование", "")).lower():
    vendor_recs.append(
        "Шифрование и защита данных: Для защиты конфиденциальной информации рекомендуется внедрение решений "
        "Symantec, Thales, Imperva, Гарда. "
        "Важно обеспечить управление ключами, шифрование данных на дисках и маскирование в БД."
    )

# --- PAM ---
if "нет" in str(results.get("PAM", "")).lower():
    vendor_recs.append(
        "Контроль привилегированного доступа (PAM): Рекомендуется внедрение решений "
        "CyberArk, WALLIX, Axidian, Netwrix, Fudo или JumpServer. "
        "Это позволит контролировать действия администраторов, вести аудит и снижать риск внутренних инцидентов."
    )

# --- BACKUP ---
if "нет" in str(results.get("Резервное копирование", "")).lower():
    vendor_recs.append(
        "Резервное копирование: Необходимо внедрение системы резервного копирования уровня enterprise. "
        "Рекомендуемые решения: Veeam, Commvault, Veritas. "
        "Обязательное требование — схема 3-2-1, наличие immutable-копий и регулярное тестирование восстановления."
    )

# --- DB SECURITY ---
if "нет" in str(results.get("Защита БД", "")).lower():
    vendor_recs.append(
        "Защита баз данных: Для мониторинга и защиты СУБД рекомендуется использование решений Imperva или Гарда. "
        "Это позволит контролировать доступ, выявлять аномалии и предотвращать утечки данных."
    )

# --- NGFW ---
if "нет" in str(results.get("NGFW", "")).lower():
    vendor_recs.append(
        "Сетевая безопасность (NGFW): Рекомендуется внедрение межсетевого экрана нового поколения. "
        "Рассмотреть решения: Check Point, Palo Alto, Fortinet, Forcepoint, Cisco, Huawei. "
        "Важно учитывать поддержку IPS, SSL inspection, VPN и централизованного управления."
    )

# --- IDS/IPS ---
if "нет" in str(results.get("IDS/IPS", "")).lower():
    vendor_recs.append(
        "Системы обнаружения и предотвращения атак (IDS/IPS): Рекомендуется внедрение решений "
        "Trend Micro или Forcepoint для выявления сетевых угроз и атак."
    )

# --- WAF ---
if "нет" in str(results.get("WAF", "")).lower():
    vendor_recs.append(
        "Защита веб-приложений (WAF): При наличии внешних сервисов рекомендуется внедрение решений "
        "Check Point, Radware, A10, Cloudflare для защиты от атак уровня приложений (OWASP Top-10)."
    )

# --- Anti-DDoS ---
if "нет" in str(results.get("AntiDDoS", "")).lower():
    vendor_recs.append(
        "Защита от DDoS: Для обеспечения доступности сервисов рекомендуется использование решений "
        "F5, Radware, Check Point с возможностью фильтрации вредоносного трафика."
    )

# --- КОРРЕЛЯЦИИ (АРХИТЕКТУРА) ---

# NGFW без IPS
if "есть" in str(results.get("NGFW", "")).lower() and "нет" in str(results.get("IDS/IPS", "")).lower():
    vendor_recs.append(
        "Текущий NGFW используется без полноценной функции предотвращения атак. "
        "Рекомендуется активировать IPS-профили или расширить функциональность используемого решения."
    )

# виртуализация без backup
if results.get("1.3.2. Виртуальные серверы", 0) > 10 and \
   "нет" in str(results.get("Резервное копирование", "")).lower():
    vendor_recs.append(
        "Виртуальная инфраструктура требует специализированной защиты. "
        "Рекомендуется использование решений Veeam или аналогов с глубокой интеграцией в гипервизор."
    )

# DLP + нет классификации
if "нет" in str(results.get("Классификация данных", "")).lower():
    vendor_recs.append(
        "Перед внедрением DLP рекомендуется провести классификацию данных, "
        "чтобы корректно определить политики защиты и снизить количество ложных срабатываний."
    )
