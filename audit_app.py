...
st.title("📋 Опросник: Технический аудит ИТ и ИБ (2026) v4.8")
...
if st.toggle("Своя сеть", key="net_toggle"):
    ...
    with n1:
        r_count = st.number_input("Маршрутизаторы (шт):", min_value=0, step=1)
        r_vendor = st.text_input("Вендор маршрутизаторов:", key="rvend")
        if r_count > 0: data['1.2.3. Маршрутизаторы'] = f"{r_count} шт. ({r_vendor})"
    with n2:
        l3_count = st.number_input("L3 Коммутаторы (шт):", min_value=0, step=1)
        l3_vendor = st.text_input("Вендор L3:", key="l3vend")
        if l3_count > 0: data['1.2.4. L3 Коммутаторы'] = f"{l3_count} шт. ({l3_vendor})"
    with n3:
        l2_count = st.number_input("L2 Коммутаторы (шт):", min_value=0, step=1)
        l2_vendor = st.text_input("Вендор L2:", key="l2vend")
        if l2_count > 0: data['1.2.5. L2 Коммутаторы'] = f"{l2_count} шт. ({l2_vendor})"
    ...
    if st.checkbox("Wi-Fi"):
        ...
        data['Wi-Fi Контроллер'] = st.text_input("Вендор/Модель контроллера:")

    if st.checkbox("Межсетевой экран (NGFW)"):
        data['1.2.7. NGFW'] = st.text_input("Вендор NGFW (Fortinet, CheckPoint и т.д.):")
        score += 20
...
if st.toggle("Есть СХД", key="st_t"):
    data['1.4.1. Носители'] = st.multiselect("Типы дисков:", ["HDD", "SSD", "NVMe", "SCM"])
    data['1.4.2. RAID'] = st.multiselect("RAID массивы:", ["RAID 0", "RAID 1", "RAID 5", "RAID 6", "RAID 10", "RAID 50", "RAID 60"])
...
if st.toggle("ИС организации", key="is_toggle"):
    ...
    # Примечание для Блока 1
    data['Примечание (ИТ)'] = st.text_area("Примечания по ИТ-инфраструктуре:", placeholder="Доп. информация по серверам, сети или софту...")
...
if st.toggle("Средства защиты", key="ib_toggle"):
    ...
    # Примечание для Блока 2
    data['Примечание (ИБ)'] = st.text_area("Примечания по ИБ:", placeholder="Опишите особенности ваших систем защиты...")
...
if st.toggle("Web-ресурсы", key="web_toggle"):
    ...
    # Примечание для Блока 3
    data['Примечание (Web)'] = st.text_area("Примечания по Web:", placeholder="CMS, внешние интеграции и т.д.")
...
if st.toggle("Разработка", key="dev_toggle"):
    ...
    # Примечание для Блока 4
    data['Примечание (Разработка)'] = st.text_area("Примечания по Разработке:", placeholder="Методологии, стек или процессы...")
...
st.info("Khalil Audit System v4.8 | Almaty 2026")
