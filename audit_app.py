...
st.title("📋 Опросник: Технический аудит ИТ и ИБ (2026) v4.4")
...
with st.expander("📖 Инструкция по заполнению (нажмите, чтобы развернуть)"):
    ...
    * **ИТ-инфраструктура:** Отразите параметры каналов связи (включая ЕШДИ/ЕТСГО), типы дисков в СХД и версии используемых платформ.
    * **Безопасность:** Отметьте используемые инструменты защиты (DLP, SIEM, NGFW) для расчета индекса технической зрелости.

    **3. Получение результата**
    * После ввода всех данных нажмите кнопку **«Сформировать экспертный отчет»**.
    * Нажмите появившуюся кнопку **«📥 Скачать отчет»**, чтобы сохранить документ в формате Excel. В файле вы найдете автоматический расчет рисков и персональные рекомендации.
    """)
...
# Добавлены конкретные версии Windows Server
srv_os_options = [
...
selected_os_srv = st.multiselect("Выберите ОС серверов:", srv_os_options, key="ms_srv_list")
if selected_os_srv:
    for os_s in selected_os_srv:
        data[f"ОС Сервера ({os_s})"] = st.number_input(f"Количество серверов на {os_s}:", min_value=0, step=1, key=f"srv_cnt_{os_s}")
...
if st.toggle("Используются внутренние ИС", key="is_block_toggle"):
    ...
    with col_is1:
        mail_options = ["Exchange (On-Prem)", "Lotus", "Microsoft 365", "Google Workspace", "Yandex/Mail.ru Cloud", "Собственный сервер", "Нет"]
        mail_sys_choice = st.selectbox("Тип почты:", mail_options, key="mail_sys")
        
        if mail_sys_choice in ["Exchange (On-Prem)", "Lotus"]:
            mail_version = st.text_input(f"Укажите версию {mail_sys_choice}:", key="mail_ver_input")
            data['1.5.1. Почтовая система'] = f"{mail_sys_choice} (v. {mail_version})" if mail_version else mail_sys_choice
        else:
            data['1.5.1. Почтовая система'] = mail_sys_choice

        if st.checkbox("Используется мониторинг?", key="mon_chk"):
            data['1.5.3. Мониторинг'] = st.selectbox("Выберите систему:", ["Zabbix", "Nagios", "PRTG", "Prometheus", "Другое"], key="mon_sel")
    with col_is2:
        st.write("**Прикладные системы (ERP/CRM/EDMS):**")
        is_list = {"1С (Бухгалтерия/ERP)": "1c", "Битрикс24": "b24", "Documentolog": "doc", "SAP": "sap", "Directum": "dir", "HelpDesk/ServiceDesk": "hd"}
        ...
        if st.checkbox("Другие системы", key="other_is_chk"):
            data['1.5.4. Другие ИС'] = st.text_input("Укажите через запятую:", key="other_is_input")

st.divider()

# --- БЛОК 2: ИНФОРМАЦИОННАЯ БЕЗОПАСНОСТЬ (РАСШИРЕННЫЙ) ---
st.header("Блок 2: Информационная Безопасность")
if st.toggle("Средства защиты", key="ib_toggle"):
    ib_systems = {
        "EPP (Антивирус)": 10,
        "DLP (Защита от утечек)": 15,
        "PAM (Упр. привилегиями)": 10,
        "SIEM (Мониторинг событий)": 20,
        "VM (Упр. уязвимостями)": 10,
        "EDR/XDR (Защита точек)": 15,
        "WAF (Защита веб)": 10,
        "Sandbox (Песочница)": 5,
        "IDS/IPS (Обнаружение атак)": 5,
        "IDM/IGA (Упр. доступом)": 5
    }
    ...
    for i, (label, pts) in enumerate(items):
        ...
        with target_col:
            if st.checkbox(label, key=f"ib_{label}"):
                v_n = st.text_input(f"Вендор/ПО {label}:", key=f"vn_{label}")
...
def make_expert_excel(c_info, results, final_score):
    ...
    score_cell.font = Font(bold=True); current_row += 2
    ...
    rec_map = {
        ...
        "Windows XP/Vista/7/8": "Критично! Устаревшие ОС. Срочно заменить.",
        "Windows Server 2008/2012 R2": "Критично! Срок поддержки истек. Требуется миграция.",
        "Windows 10": "Внимание! ОС требует планового обновления.",
        "Windows 11": "Нормально. Соблюдать график обновлений."
    }

    for k, v in results.items():
        ws.cell(row=current_row, column=1, value=k).border = border
        ws.cell(row=current_row, column=2, value=str(v)).border = border
        status, recommendation, is_risk, is_warning = "В норме", "Поддерживать состояние.", False, False
        
        if "Нет" in str(v) or v == 0 or v == []:
            is_risk = True
            recommendation = rec_map.get(k, "Рассмотреть возможность внедрения.")
        
        if ("Windows XP/Vista/7/8" in k or "2008/2012 R2" in k) and v > 0:
            is_risk = True; recommendation = rec_map.get("Windows XP/Vista/7/8", "Устаревшая версия.")
        elif "Windows 10" in k and v > 0:
            is_warning = True; recommendation = rec_map["Windows 10"]

        st_cell = ws.cell(row=current_row, column=3, value="РИСК" if is_risk else "ПРЕДУПРЕЖДЕНИЕ" if is_warning else status)
        if is_risk: st_cell.font = Font(color="FF0000", bold=True)
        elif is_warning: st_cell.font = Font(color="FFC000", bold=True)
...
if st.button("📊 Сформировать экспертный отчет", key="btn_final"):
    ...
    if not all(mandatory):
        ...
        with st.spinner("Создаем отчет..."):
            ...
            try:
                caption = f"🚀 *Новый заказ Khalil Trade*\n🏢 *Компания:* {client_info['Наименование компании']}\n📊 *Зрелость:* {f_score}%\n👤 *Контакт:* {client_info['ФИО контактного лица']}"
...
st.info("Khalil Audit System v4.4 | Almaty 2026")
