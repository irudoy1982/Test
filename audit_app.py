...
def render_audit_presentation_template(template_path, replacements):
    ...
    for visible_number, slide_number in enumerate(active_slide_numbers, start=1):
        ...
        if shape_tree is not None and slide_number == 3:
            visible_outcomes = min(recommendation_count, 4)
            outcome_rows = (
                (1900000, 2900000),
                (2920000, 3930000),
                (3950000, 4960000),
            ...
            for shape in list(shape_tree):
                ...
                row_index = next(
                    (index for index, (top, bottom) in enumerate(outcome_rows, start=1) if top <= y_position < bottom),
                    None,
                )
                if row_index is not None and row_index > visible_outcomes:
