...
def make_expert_excel(c_info, results, final_score):
    ...
    for k, v in results.items():
        ...
        # Безопасное приведение к строке для поиска подстрок
        val_str = str(v).lower()
        # Проверка: является ли значение числом больше 0 (для ОС и АРМ)
        is_numeric_positive = isinstance(v, (int, float)) and v > 0

        # 1. Анализ ОС (Устаревшие системы)
        if any(old in k for old in ["XP", "7", "8", "2008", "2012"]) and is_numeric_positive:
            status = "КРИТИЧНО"
            rec = "Обнаружены ОС с истекшим сроком поддержки (EoL). Высокий риск эксплуатации 0-day уязвимостей. Необходима срочная миграция."
        elif "Windows 10" in k and is_numeric_positive:
            status = "ВНИМАНИЕ"
            rec = "Windows 10 приближается к завершению жизненного цикла. Рекомендуется плановое обновление до Windows 11."

        # 2. Сетевая безопасность и отказоустойчивость
        elif "Резервный канал" in k and "нет" in val_str:
            status = "ВЫСОКИЙ РИСК"
            rec = "Отсутствует резервный канал связи. Рекомендуется подключение второго провайдера для обеспечения непрерывности бизнеса."
        elif "NGFW" in k and "нет" in val_str:
            status = "ВЫСОКИЙ РИСК"
            rec = "Периметр не защищен NGFW. Рекомендуется внедрение шлюза безопасности с функциями IPS/IDS и глубокого анализа трафика."

        # 3. Резервное копирование и ИБ
        elif "Резервное копирование" in k and ("нет" in val_str or not v):
            status = "КРИТИЧНО"
            rec = "Отсутствует СРК. Требуется внедрение системы бэкапа по правилу 3-2-1 (Veeam, Кибербекап) для защиты от шифровальщиков."
        elif k in ["EPP (Антивирус)", "EDR/XDR (Точки)"] and "нет" in val_str:
            status = "КРИТИЧНО"
            rec = "Конечные точки беззащитны. Необходимо развертывание антивируса корпоративного класса с централизованным управлением."
        elif "MFA (Аутентификация)" in k and "нет" in val_str:
            status = "ВЫСОКИЙ РИСК"
            rec = "Риск захвата учетных записей. Рекомендуется внедрение двухфакторной аутентификации для VPN и критичных систем."

        # 4. Веб и Разработка
        elif "CICD" in k and v is False:
            status = "ВНИМАНИЕ"
            rec = "Ручное развертывание увеличивает риск ошибок (human factor). Рекомендуется автоматизация процессов сборки и деплоя."

        # Общие правила для пустых полей и примечаний
        if "Примечание" in k:
            status = "ИНФО"
            rec = "Дополнительная информация от заказчика принята к сведению."
        elif v == 0 or "нет" in val_str or not val_str.strip():
            if status == "В норме":
                status = "РИСК"
                rec = "Параметр не заполнен или отсутствует. Требуется оценка целесообразности внедрения данного функционала."
    ...
    # Настройка ширины колонок и переноса текста
    widths = {'A': 40, 'B': 30, 'C': 20, 'D': 70}
